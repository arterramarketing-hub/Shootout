...
/**
 * Give every triangle its own three vertices and one normal.
 *
 * Smooth normals blend the light across a joint so a limb reads as round;
 * flat ones light each face on its own so the same limb reads as the
 * handful of planes it actually is. The second is the look of a figure from
 * the hardware the retro mode is after, and it is what makes a six-sided
 * arm look like a deliberate six-sided arm rather than a round one drawn
 * badly. Colours and bones ride along from the vertex each corner came from.
 */
export const flatten = (skin: Skin): Skin => {
  const out: Skin = {
    positions: [],
    normals: [],
    colors: [],
  ...
  for (let i = 0; i < skin.indices.length; i += 3) {
    const corners = [skin.indices[i], skin.indices[i + 1], skin.indices[i + 2]];
    const at = (index: number): Vec3 => ({
      x: skin.positions[index * 3],
      y: skin.positions[index * 3 + 1],
      z: skin.positions[index * 3 + 2],
    });
    // The loft winds so the cross of the edges points into the surface.
    const face = cross(sub(at(corners[1]), at(corners[0])), sub(at(corners[2]), at(corners[0])));
    const length = Math.hypot(face.x, face.y, face.z) || 1;
    const nx = -face.x / length;
    const ny = -face.y / length;
    const nz = -face.z / length;
    for (const index of corners) {
      out.indices.push(out.positions.length / 3);
      out.positions.push(
        skin.positions[index * 3],
        skin.positions[index * 3 + 1],
        skin.positions[index * 3 + 2],
      );
      out.normals.push(nx, ny, nz);
      for (let k = 0; k < 4; k += 1) {
        out.colors.push(skin.colors[index * 4 + k]);
        out.boneIndices.push(skin.boneIndices[index * 4 + k]);
        out.boneWeights.push(skin.boneWeights[index * 4 + k]);
      }
    }
  }
  return out;
